...
class ClipMatcher(SetCriterion):
    def __init__(self, num_classes,
        ...
        if 'loss_contrastive' not in losses:
    ...
    def loss_refers(self,  outputs, gt_instances: List[Instances], indices: List[tuple], num_boxes):
        ...
        return losses
    # models/transrmot_pro.py -> class SetCriterion

    # models/transrmot_pro.py -> SetCriterion

    # 请确保将此方法放入 class ClipMatcher(nn.Module): 内部
    def loss_contrastive(self, outputs, targets, indices, num_boxes, log=True):
        """
        创新点2: 独立对比精炼损失 (InfoNCE) - 适配 ClipMatcher
        """
        # 检查输出是否存在
        if 'contrastive_visual' not in outputs:
            return {'loss_contrastive': torch.tensor(0.0).to(outputs['pred_logits'].device)}

        visual_embeds = outputs['contrastive_visual'] # [Batch, Num_Queries, 128]
        text_embeds = outputs['contrastive_text']     # [Batch, 128]
        
        # 获取匹配索引
        idx = self._get_src_permutation_idx(indices)
        batch_idx, src_idx = idx
        
        total_loss = torch.tensor(0.0).to(visual_embeds.device)
        valid_batches = 0
        
        # 遍历 Batch 计算 InfoNCE
        for b in range(len(targets)):
            current_batch_mask = (batch_idx == b)
            matched_indices = src_idx[current_batch_mask]
            ...
            # Anchor (Visual) & Positive (Text)
            anchor = visual_embeds[b][matched_indices] # [M, 128]
            positive = text_embeds[b].unsqueeze(0)     # [1, 128]
            
            # Negative (Background Queries)
            all_indices = torch.arange(visual_embeds.shape[1], device=visual_embeds.device)
            neg_mask = torch.ones(visual_embeds.shape[1], dtype=torch.bool, device=visual_embeds.device)
            neg_mask[matched_indices] = False
            negative = visual_embeds[b][all_indices[neg_mask]] # [K, 128]

            # Logits
            l_pos = torch.mm(anchor, positive.t()) / self.tau
            l_neg = torch.mm(anchor, negative.t()) / self.tau
...
class TransRMOT(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
        ...
        self.contrastive_proj_img = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(transformer.d_model, 128)
        )
    ...
    def _forward_single_image(self, samples, track_instances: Instances, sentences):
        ...
        detached_query_feats = last_query_feats.detach()
        visual_embeds = self.contrastive_proj_img(detached_query_feats)
        visual_embeds = F.normalize(visual_embeds, p=2, dim=-1)
        if text_sentence_features.dim() == 3:
             text_proto = text_sentence_features.mean(dim=0) # -> [1, 256]
        else:
             text_proto = text_sentence_features
             
        text_embeds = self.contrastive_proj_text(text_proto) # -> [1, 128]
...
def build(args):
    ...
    for i in range(num_frames_per_batch):
        weight_dict.update({"frame_{}_loss_ce".format(i): args.cls_loss_coef,
                            ...
                            'frame_{}_loss_contrastive'.format(i): 2.0,
                            ...
                            'frame_{}_temporal_loss_contrastive'.format(i): 2.0,
                            })
